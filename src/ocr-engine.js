 ...
 * Implements an embedded lightweight micro-OCR engine for browser execution that:
 * 1. Segments candidate text regions into character glyphs.
 * 2. Uses Stroke-Width Transform (SWT) & contour structure filtering to eliminate false positives on anatomical edges (TASK-06).
 * 3. Matches glyph features & topological masks against embedded character font templates (A-Z, 0-9, symbols).
 * 4. Includes performance downscaling & frame timeout guards for slower devices.
 */
  ...
  paddleOcrPromise = (async () => {
    if (typeof window !== "undefined" && window.PaddleOCR) {
      try {
        ...
        // OCR errors may include URLs or model diagnostics. Callers that need
        // diagnostics can opt in through their own non-PHI telemetry.
      }
    }
    return null;
  })();
...
/**
 * Embedded 7x9 Alphanumeric & Symbol Font Templates
 * Used for micro-OCR character recognition in 100% browser-based execution.
 */
...
// Convert string template representations into 7x7 binary arrays (1 or 0)
...
/**
 * Matches a 2D binary glyph patch against compiled font character templates.
 * Returns best matching character and confidence score.
 */
export function matchGlyphToCharacter(glyphData, glyphW, glyphH) {
  ...
  // Downsample/resample glyph patch to standard 7x7 grid
...
/**
 * Stroke-Width Transform (SWT) & Edge Contour Topology Filter.
 * Used to filter out high-contrast anatomical edges (e.g. cortical bone contours, diaphragm, surgical clips)
 * which trigger false positive text box detections (TASK-06).
 */
export function isAnatomicalEdgeNotText(binaryPatch, patchW, patchH) {
  if (patchW < 8 || patchH < 6) return false;

  // Calculate stroke width variance across horizontal lines in the patch
  const strokeWidths = [];
  ...
  // Average and variance of stroke widths
  ...
  // Anatomical edges have large, non-uniform stroke widths (> 45% of patch height or variance > 25)
  if (mean > patchH * 0.45 || (mean > 12 && variance > 25)) {
    return true; // Anatomical contour edge
  }
...
/**
 * Scans ImageData for rendered burned-in text overlays, patient names, dates, and annotations.
 * Uses micro-OCR character recognition to extract real text strings.
 * Includes performance downscaling & bounds capping for slower hardware.
 */
export function scanImageDataForText(imageData, frameIndex = 0, options = {}) {
  ...
  // Check hardware downscaling limit for slower devices
  ...
  if (options.ocrPerformanceMode === "fast" || width > maxRes || height > maxRes) {
    ...
    if (scale < 1.0) {
      ...
      // Resample RGBA data to downscaled dimensions for fast processing
  ...
  // Merge connected grid cells into raw candidate boxes
  ...
  for (const b of rawBoxes) {
    ...
    // Extract binary patch for Stroke-Width Transform & Anatomical Edge check (TASK-06)
    ...
    // Run micro-OCR character recognition across the text region
...
/**
 * Micro-OCR Character Recognition Engine:
 * Segments candidate binary patch into character glyphs and performs template matching.
 */
function recognizeBoxTextWithMicroOcr(patch, patchW, patchH, boxX, boxY, imageW, imageH, options) {
  // Segment candidate patch into character glyphs along vertical projection valleys
 ...
 * Attempts WASM PaddleOCR execution when available; falls back to embedded Micro-OCR engine.
 */
export async function runOcrOnFrame(imageData, frameIndex = 0, options = {}) {
  ...
  // Optional user prompt/confirmation when running fallback micro-OCR on slower hardware
